 ...
 */
export default function AirConsole(opts) {
  this.init_(opts);
}
/**
...
AirConsole.prototype.getCustomDeviceState = function(device_id) {
  ...
  if (
    device_data &&
    this.getGameUrl_(this.getLocationUrl_()) ==
 ...
 */
AirConsole.prototype.onCustomDeviceStateChange = function(
  device_id,
  custom_data
) {};
/**
...
AirConsole.prototype.getProfilePicture = function(device_id_or_uid, size) {
  if (device_id_or_uid === undefined) {
    ...
    return (
      "https://www.airconsole.com/api/profile-picture?uid=" +
      device_id_or_uid +
      "&size=" +
      (size || 64)
    );
...
AirConsole.prototype.convertDeviceIdToPlayerNumber = function(device_id) {
  if (
    !this.devices[AirConsole.SCREEN] ||
 ...
 */
AirConsole.prototype.storeHighScore = function(
  level_name,
  level_version,
  score,
  uid,
 ...
 */
AirConsole.prototype.requestHighScores = function(
  level_name,
  level_version,
  uids,
  ranks,
...
AirConsole.prototype.onPostMessage_ = function(event) {
  ...
  if (data.action == "device_motion") {
    ...
    if (me.device_id !== undefined) {
      ...
      var is_connect =
        game_url_before != game_url && game_url_after == game_url;
      if (is_connect) {
        me.onConnect(data.device_id);
      } else if (game_url_before == game_url && game_url_after != game_url) {
        me.onDisconnect(data.device_id);
      }
      if (data.device_data) {
        if (
          (data.device_data._is_custom_update && game_url_after == game_url) ||
          (is_connect && data.device_data.custom)
        ) {
          me.onCustomDeviceStateChange(data.device_id, data.device_data.custom);
        }
        if (
          (data.device_data._is_players_update && game_url_after == game_url) ||
          (data.device_id == AirConsole.SCREEN &&
            data.device_data.players &&
            is_connect)
        ) {
          me.device_id_to_player_cache = null;
          me.onActivePlayersChange(
            me.convertDeviceIdToPlayerNumber(me.getDeviceId())
          );
        }
        if (
    ...
    for (var i = 0; i < me.devices.length; ++i) {
      if (me.devices[i] && me.getGameUrl_(me.devices[i].location) == game_url) {
        if (i != me.getDeviceId()) {
          me.onConnect(i);
...
(function() {
  if (window.Proxy) {
    var r = function(a, b, c, e, f) {
        a._xmlHttpRequestRetryRanger.pending = !0;
        a == b &&
          b.addEventListener("loadstart", function(b) {
            g(a._xmlHttpRequestRetryRanger.listeners.loadstart, b);
          });
        b.addEventListener("readystatechange", function(d) {
          if (l(a, b) && 4 == b.readyState) {
            a._xmlHttpRequestRetryRanger.pending = !1;
            if (
              a._xmlHttpRequestRetryRanger.config.USE_PSEUDO_RANGE_HEADERS
                ? b.getResponseHeader("x-retry-ranger-content-range")
                : b.getResponseHeader("content-range")
            )
              if ("arraybuffer" == b.responseType)
                a._xmlHttpRequestRetryRanger.response.set(
                ...
                if (
                  ...
                  a._xmlHttpRequestRetryRanger.response += b.response;
              }
            else throw "Response didn't include Content-Range header";
            p(a, d, !1);
            if (e < a._xmlHttpRequestRetryRanger.total - 1) {
              d = new k();
              d.responseType = b.responseType;
              var f = Math.min(
                e + a._xmlHttpRequestRetryRanger.config.CHUNK_SIZE,
                a._xmlHttpRequestRetryRanger.total - 1
              );
              q(a, d, e + 1, f);
              r(a, d, e + 1, f);
              f == a._xmlHttpRequestRetryRanger.total - 1 && v(a, d);
              d.send();
            }
          }
        });
        a._xmlHttpRequestRetryRanger.lastLoaded = 0;
        b.addEventListener("progress", function(c) {
          if (0 != b.status) {
            var e = c.loaded - a._xmlHttpRequestRetryRanger.lastLoaded;
            a._xmlHttpRequestRetryRanger.lastLoaded += e;
            a._xmlHttpRequestRetryRanger.loaded += e;
            c = h("progress", c, a);
            a._xmlHttpRequestRetryRanger.progressComplete = c.loaded == c.total;
            g(a._xmlHttpRequestRetryRanger.listeners.progress, c);
          }
        });
        b.addEventListener("error", function(d) {
          a._xmlHttpRequestRetryRanger.loaded -=
            a._xmlHttpRequestRetryRanger.lastLoaded;
          a._xmlHttpRequestRetryRanger.lastLoaded = 0;
          var u = a._xmlHttpRequestRetryRanger.retries;
          u < a._xmlHttpRequestRetryRanger.config.RETRY
            ? window.setTimeout(function() {
                var d = new k();
                f && w(a, d, c, e);
                d.responseType = b.responseType;
                a._xmlHttpRequestRetryRanger.retries += u + 1;
                q(a, d, c, e);
                r(a, d, c, e, f);
                0 != c &&
                  e == a._xmlHttpRequestRetryRanger.total - 1 &&
                  v(a, d);
                d.send();
              }, a._xmlHttpRequestRetryRanger.config.RETRY_BACKOFF)
            : ((a._xmlHttpRequestRetryRanger.readyState = 4),
              (a._xmlHttpRequestRetryRanger.status = 0),
              g(
                a._xmlHttpRequestRetryRanger.listeners.readystatechange,
                z("readystatechange", d, a)
              ),
              g(
                a._xmlHttpRequestRetryRanger.listeners.progress,
                h("progress", d, a)
              ),
              g(a._xmlHttpRequestRetryRanger.listeners.error, h("error", d, a)),
              g(
                a._xmlHttpRequestRetryRanger.listeners.loadend,
                h("loadend", d, a)
              ));
        });
        x(a, "timeout");
      ...
      v = function(a, b) {
        b.addEventListener("readystatechange", function(c) {
          4 == b.readyState && 0 != b.status && p(a, c, !0);
        });
      },
      w = function(a, b, c, e) {
        a._xmlHttpRequestRetryRanger.retries = 0;
        b.addEventListener("readystatechange", function(c) {
          if (2 == b.readyState)
            if (
              ((a._xmlHttpRequestRetryRanger.status = b.status),
              (a._xmlHttpRequestRetryRanger.statusText = b.statusText),
              l(a, b))
            ) {
              var d;
              if (
                (d = a._xmlHttpRequestRetryRanger.config
                  .USE_PSEUDO_RANGE_HEADERS
                  ? b.getResponseHeader("x-retry-ranger-content-range")
                  : b.getResponseHeader("content-range"))
              ) {
                d = d.split("/")[1];
                if ("*" == d)
                  throw "Response didn't include total Content-Range size";
                d = parseInt(d);
              } else throw "Response didn't include Content-Range header";
              a._xmlHttpRequestRetryRanger.total = d;
              a._xmlHttpRequestRetryRanger.firstRequest = b;
              if ("arraybuffer" == b.responseType)
                a._xmlHttpRequestRetryRanger.response = new Uint8Array(d);
              else if (
                "" == b.responseType ||
                "text" == b.responseType ||
                "json" == b.responseType
              )
                a._xmlHttpRequestRetryRanger.response = "";
              else throw "responseType " + b.responseType + " not supported";
            } else
              (a._xmlHttpRequestRetryRanger.total = 0),
                (a._xmlHttpRequestRetryRanger.response = ""),
                (a._xmlHttpRequestRetryRanger.firstRequest = b);
          4 == b.readyState
            ? 0 != b.status &&
              (e >= a._xmlHttpRequestRetryRanger.total - 1 || !l(a, b)) &&
              (l(a, b) || (a._xmlHttpRequestRetryRanger.pending = !1),
              p(a, c, !0))
            : a._xmlHttpRequestRetryRanger.readyState < b.readyState &&
              ((a._xmlHttpRequestRetryRanger.readyState = b.readyState),
              g(
                a._xmlHttpRequestRetryRanger.listeners.readystatechange,
                m(c, a)
      ...
      q = function(a, b, c, e) {
        for (
          var f = [], d = 0;
          d < a._xmlHttpRequestRetryRanger.open_arguments.length;
          ++d
        )
          1 != d
            ? f.push(a._xmlHttpRequestRetryRanger.open_arguments[d])
            : f.push(
                a._xmlHttpRequestRetryRanger.config.URL_REWRITE(
                  a._xmlHttpRequestRetryRanger.open_arguments[d],
                  c,
                  e,
                  a._xmlHttpRequestRetryRanger.retries
                )
              );
        f = b.open.apply(b, f);
        d = "range";
        a._xmlHttpRequestRetryRanger.config.USE_PSEUDO_RANGE_HEADERS &&
          (d = "x-retry-ranger-range");
        b.setRequestHeader(d, "bytes=" + c + "-" + e);
        return f;
      },
      p = function(a, b, c) {
        c && (a._xmlHttpRequestRetryRanger.readyState = 4);
        4 != a._xmlHttpRequestRetryRanger.readyState ||
          a._xmlHttpRequestRetryRanger.pending ||
          (a._xmlHttpRequestRetryRanger.progressComplete ||
            g(
              a._xmlHttpRequestRetryRanger.listeners.progress,
              h("progress", b, a, !0)
            ),
          g(a._xmlHttpRequestRetryRanger.listeners.readystatechange, m(b, a)),
          g(a._xmlHttpRequestRetryRanger.listeners.load, h("load", b, a, !0)),
          g(
            a._xmlHttpRequestRetryRanger.listeners.loadend,
            h("loadend", b, a, !0)
          ));
      ...
      x = function(a, b) {
        a.addEventListener(b, A(a, b));
      },
      A = function(a, b) {
        return function(c) {
          g(a._xmlHttpRequestRetryRanger.listeners[b], c);
        };
      },
      g = function(a, b) {
        if (a.on) a.on(b);
        for (var c = 0; c < a.listeners.length; ++c) a.listeners[c](b);
      },
      h = function(a, b, c, e) {
        return new Proxy(m(b, c), {
          get: function(b, d) {
            return "lengthComputable" == d
              ? 0 < c._xmlHttpRequestRetryRanger.total
              : "total" == d
              ? c._xmlHttpRequestRetryRanger.total
              : "loaded" == d
              ? e
                ? c._xmlHttpRequestRetryRanger.total
                : c._xmlHttpRequestRetryRanger.loaded
              : "type" == d
              ? a
              : b[d];
          },
      ...
      m = function(a, b) {
        return new Proxy(a, {
          get: function(a, e) {
            return "target" == e || "currentTarget" == e
              ? b._xmlHttpRequestRetryRanger.proxy
              : "function" == typeof a[e]
              ? function() {
      ...
      t = {
        ...
        ACTIVE: function() {
          var a = ["Chrome", "Safari", "Firefox", "Edge"];
          if (navigator.userAgent)
            for (var b = 0; b < a.length; ++b)
              if (-1 != navigator.userAgent.indexOf(a[b] + "/")) return !0;
          return !1;
        },
        RETRY: 3,
        RETRY_BACKOFF: 1e3,
        URL_REWRITE: function(a, b, c, e) {
          return a;
        },
        USE_PSEUDO_RANGE_HEADERS: !1
      },
      n = {
        loadstart: !0,
        progress: !0,
      ...
      B = {
        set: function(a, b, c) {
          0 == b.indexOf("on") &&
          void 0 !== a._xmlHttpRequestRetryRanger.listeners[b.substr(2)]
            ? ((a = a._xmlHttpRequestRetryRanger.listeners[b.substr(2)]),
              a.on && delete a.on,
              c && (a.on = c))
            : ("responseType" == b &&
                ((a._xmlHttpRequestRetryRanger.responseType = c),
                "json" == c && (c = "")),
              (a[b] = c));
          return !0;
        },
        get: function(a, b) {
          var c = !!a._xmlHttpRequestRetryRanger.open_arguments;
          if ("open" == b)
            return function() {
              var c = arguments[1];
              if (
                "GET" == arguments[0].toUpperCase() &&
                !1 !== arguments[2] &&
                a._xmlHttpRequestRetryRanger.config.MATCH(c)
              ) {
                a._xmlHttpRequestRetryRanger.open_arguments = arguments;
                c = a._xmlHttpRequestRetryRanger.config.CHUNK_SIZE - 1;
                w(a, a, 0, c);
                var f = q(a, a, 0, c);
                r(a, a, 0, c, !0);
                return f;
              }
              a.responseType = a._xmlHttpRequestRetryRanger.responseType || "";
              for (f in n) x(a, f);
              return a[b].apply(a, arguments);
            };
          if ("addEventListener" == b)
            return function() {
              if (void 0 !== n[arguments[0]])
                a._xmlHttpRequestRetryRanger.listeners[
                  arguments[0]
                ].listeners.push(arguments[1]);
              else return a[b].apply(a, arguments);
            };
          if ("removeEventListener" == b)
            return function() {
              if (void 0 !== n[arguments[0]])
                for (
                  var c =
                      a._xmlHttpRequestRetryRanger.listeners[arguments[0]]
                        .listeners,
                    f = 0;
                  f < c.length;
                  ++f
                )
                  c[f] == arguments[1] && c.splice(f);
              else return a[b].apply(a, arguments);
            };
          if ("responseText" == b && c) {
            if (
              "text" != a._xmlHttpRequestRetryRanger.responseType &&
              a._xmlHttpRequestRetryRanger.responseType
            )
              throw "responseText not available with response type " +
                a._xmlHttpRequestRetryRanger.responseType;
            return 4 == a._xmlHttpRequestRetryRanger.readyState
              ? a._xmlHttpRequestRetryRanger.response || ""
              : "";
          }
          if ("response" == b && c) {
            if (4 == a._xmlHttpRequestRetryRanger.readyState)
              if (
                "text" != a._xmlHttpRequestRetryRanger.responseType &&
                a._xmlHttpRequestRetryRanger.responseType
              ) {
                if ("json" == a._xmlHttpRequestRetryRanger.responseType)
                  return JSON.parse(a._xmlHttpRequestRetryRanger.response);
                if ("arraybuffer" == a._xmlHttpRequestRetryRanger.responseType)
                  return a._xmlHttpRequestRetryRanger.response.buffer;
              } else return a._xmlHttpRequestRetryRanger.response || "";
            return "json" == a._xmlHttpRequestRetryRanger.responseType
              ? null
              : "";
          }
          if ("responseType" == b && c)
            return a._xmlHttpRequestRetryRanger.responseType || "";
          if ("readyState" == b && c)
            return a._xmlHttpRequestRetryRanger.readyState || 0;
          if ("status" == b && c)
            return 206 == a._xmlHttpRequestRetryRanger.status
              ? 200
              : a._xmlHttpRequestRetryRanger.status || 0;
          if ("statusText" == b && c)
            return "Partial Content" == a._xmlHttpRequestRetryRanger.statusText
              ? "OK"
              : a._xmlHttpRequestRetryRanger.statusText || "";
          if ("responseURL" == b && c)
            return (c = a._xmlHttpRequestRetryRanger.firstRequest) ? c[b] : "";
          if ("abort" == b || "overrideMimeType" == b)
            console.error(
              b + " is not implemented in xmlhttprequest-retry-ranger"
            );
          else
            return "getResponseHeader" == b && c
              ? function() {
                  var c = a._xmlHttpRequestRetryRanger.firstRequest;
                  return c
                    ? arguments[0] &&
                      "content-range" == arguments[0].toLowerCase()
                      ? null
                      : arguments[0] &&
                        "content-length" == arguments[0].toLowerCase()
                      ? void 0 !== a._xmlHttpRequestRetryRanger.total
                        ? a._xmlHttpRequestRetryRanger.total
                        : null
                      : c[b].apply(c, arguments)
                    : null;
                }
              : "getAllResponseHeaders" == b && c
              ? function() {
                  var b = "",
                    c = a._xmlHttpRequestRetryRanger.firstRequest;
                  if (c) {
                    c = c.getAllResponseHeaders().split("\r\n");
                    for (var d = 0; d < c.length; ++d) {
                      var g = c[d].split(":");
                      if (2 <= g.length) {
                        g = g[0];
                        var h = a._xmlHttpRequestRetryRanger.proxy.getResponseHeader(
                          g
                        );
                        null != h && (b += g + ": " + h + "\r\n");
                      }
    ...
    window.XMLHttpRequest = function(a) {
      a = new k(a);
      var b = window.xmlHttpRequestRetryRanger || t;
      if (window.xmlHttpRequestRetryRanger)
        for (var c in t) {
          var e = window.xmlHttpRequestRetryRanger[c];
          b[c] = void 0 !== e ? e : t[c];
        }
      ...
      for (var f in n) b.listeners[f] = { listeners: [] };
      a._xmlHttpRequestRetryRanger = b;
      return c;
    };
    for (var y in k) window.XMLHttpRequest[y] = k[y];
  }
})();
...
(function() {
  if (window.Proxy) {
    ...
    window.XMLHttpRequest = function(c) {
      var e,
        f,
        g = new d(c);
      if (window.xmlHTTPRequestGlobalCallbacks)
        for (var h in window.xmlHTTPRequestGlobalCallbacks)
          (function(a) {
            g.addEventListener(a, function(b) {
              window.xmlHTTPRequestGlobalCallbacks[a](b, e, f);
            });
          })(h);
      return new Proxy(g, {
        get: function(a, b) {
    ...
    for (var c in d) window.XMLHttpRequest[c] = d[c];
  }
})();

window.xmlHttpRequestRetryRanger = {
  CHUNK_SIZE: 8388608,
  MATCH: function(a) {
    return (
      (0 == a.indexOf("Build/") && a.endsWith(".unityweb")) ||
      (-1 != a.indexOf("StreamingAssets/") && a.endsWith(".unity3d")) ||
      (0 == a.indexOf("Release/") &&
        (a.endsWith(".js") ||
          a.endsWith(".jsgz") ||
          a.endsWith(".mem") ||
  ...
  ACTIVE: function() {
    if (
      (window.onGameReady || window.UnityLoader) &&
      "game.airconsole.com" == document.location.hostname &&
      document.location.pathname.endsWith("screen.html")
    ) {
      var a = ["Chrome", "Safari", "Firefox", "Edge"];
      if (navigator.userAgent)
        for (var b = 0; b < a.length; ++b)
  ...
  USE_PSEUDO_RANGE_HEADERS: !0,
  URL_REWRITE: function(a, b, c, d) {
    d = a.split("#");
    var e = d[0];
    a = void 0;
    void 0 != d[1] && (d.shift(), (a = d.join("#")));
    e = -1 == e.indexOf("?") ? e + "?" : e + "&";
    b =
      e +
      ("xmlhttprequest-ranger-start=" + b + "&xmlhttprequest-ranger-end=" + c);
    void 0 != a && (b += "#" + a);
    return b;
  }
};
  ...
  error: function(a, b, c) {
    c.match(/\.unityweb$/) &&
      ...
      (a.onload = function() {
        AirConsole.postMessage_({
          action: "set",
          key: "xmlhttprequest_status",
          value: { status: "error", url: c }
        });
      }),
      a.open(
        "GET",
        "/games.airconsole.com/cdn/cdn.js?ts=" + new Date().getTime(),
        !0
      ),
      a.send());
  },
